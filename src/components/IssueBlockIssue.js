...
export default function IssueBlockIssue(props) {
	...
	const loadingLabels = () => {
		...
		if (props.info.labels[0] == null || props.info.labels[0] == 'dependencies') {
			return
		} else return props.info.labels.map(id => { return <div><Badge class="badgess" style={{marginLeft: 10, marginTop: 10}}variant="success">{id.name}</Badge></div>})
		...
		<div>
			<div className="mg" onClick={() => clickTitle(props.info.number)}>
				<div className="flex">
					...
					<FontAwesomeIcon icon={faCommentAlt} style={{ color: '#586069', marginRight: 5, marginTop: 4 }} /><h4 style={{ fontSize: 16, textAlign: 'right' }}>{props.info.comments}</h4>
					</div>
