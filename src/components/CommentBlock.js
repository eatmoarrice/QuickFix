		...
		<table key={item.id} width="800">
			<thead>
				<tr height="50" align="left" bgcolor="#eeeeee">
					<th width="50"><img className="avatar" width="50" src={item.user.avatar_url}></img></th>
					<th width="650"style={{marginLeft:"10px",}}><i className="fa fa-caret-left"></i>{`  ${item.user.login}`}<span style={{fontWeight:"normal"}}>{` commented: ${moment(item.created_at).fromNow()}`}</span></th>
					<th></th>
					<th><button className="replyBtn" onClick={()=>reply(item.user.login)} style={{backgroundColor:"#6c757d", color:"white", border:"none", borderRadius:"15px", paddingLeft:"15px", paddingRight:"15px", fontWeight:"bold"}}>Reply</button></th>
				</tr>
			</thead>
			<tbody>
				<tr>
				...
				<tr>
					<td></td>
					<td colSpan="3" align="left" className="ArticleCopy" style={{paddingBottom:"10px", paddingLeft:"10px", borderLeft:"solid 1px #eeeeee", borderRight:"solid 1px #eeeeee", borderBottom:"solid 1px #eeeeee",}}><ReactMarkdown key={index} source={item.body}/></td>
				</tr>
				<tr height="50">
					<td></td>
					<td className="text-left" style={{paddingLeft:"10px"}}>Total Reactions</td>
					<td></td>
					<td>
						<Dropdown>
